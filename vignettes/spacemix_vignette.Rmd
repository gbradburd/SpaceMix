...
str(spacemix.dataset)

# look at the attributes of the allele counts matrix
dim(spacemix.dataset$allele.counts)
class(spacemix.dataset$allele.counts)

# look at the first 10 loci
spacemix.dataset$allele.counts[1:30,1:10]
spacemix.dataset$sample.sizes[1:30,1:10]
```
...
allele.frequency.matrix <- spacemix.dataset$allele.counts / spacemix.dataset$sample.sizes
...
run.spacemix.analysis(n.fast.reps = 10,
						...
						loci = ncol(spacemix.dataset$sample.sizes),
						ngen = 1e6,
						printfreq = 1e2,
						samplefreq = 1e3,
...
allele.frequencies <- spacemix.dataset$allele.counts / spacemix.dataset$sample.sizes
tmp.sample.sizes <- spacemix.dataset$sample.sizes
tmp.sample.sizes[which(spacemix.dataset$sample.sizes == 0)] <- NA
mean.sample.sizes <- rowMeans(tmp.sample.sizes,na.rm=TRUE)
...
run.spacemix.analysis(n.fast.reps = 10,
						...
						target.spatial.prior.scale=NULL,
						source.spatial.prior.scale=NULL,
						spatial.prior.X.coordinates = runif(nrow(spacemix.dataset$allele.counts),-180,180),
						spatial.prior.Y.coordinates = runif(nrow(spacemix.dataset$allele.counts),-90,90),
						round.earth = TRUE,
						long.run.initial.parameters=NULL,
...
allele.frequencies <- spacemix.dataset$allele.counts / spacemix.dataset$sample.sizes
normalized.allele.freqs <- normalize.allele.freqs(allele.frequencies,spacemix.dataset$sample.sizes)
sample.cov <- cov(t(normalized.allele.freqs))
...
initial.param.vals <- list("a0" = 0.9,
					 "a1" = 1.3,
					 "a2" = 0.5,
					 "population.coordinates" = rbind(spacemix.dataset$population.coordinates,
...
data(example.model.adequacy.RData)
...
#	credible interval on parameter estimates
